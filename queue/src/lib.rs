...
pub(crate) fn lua_json_array<T: DeserializeOwned>(json: &str) -> Result<Vec<T>, MessageQueueError> {
    fn normalize(value: Value) -> Value {
        match value {
            Value::Object(map) => {
                if map.keys().all(|key| key.parse::<usize>().is_ok()) {
                    let mut numeric_entries: Vec<_> = map
                        .into_iter()
                        .map(|(key, value)| (key.parse::<usize>().unwrap(), normalize(value)))
                        .collect();
                    numeric_entries.sort_by_key(|(index, _)| *index);
                    Value::Array(
                        numeric_entries
                            .into_iter()
                            .map(|(_, value)| value)
    ...
    serde_json::from_value(normalize(value)).map_err(MessageQueueError::from)
}
...
impl<H: DurableExecution> Queue<H> {
    ...
    pub fn success_list_name(&self) -> String {
        format!("queue:{}:success", self.name)
    }
                            ...
                            Ok(jobs) => {
                                ...
                                for job in jobs {
                                    ...
                                    tokio::spawn(
                                        ...
                                        drop(permit);
                                    }.instrument(tracing::info_span!("queue_worker", job_id, queue_name)));
                                }
                            }
                            Err(e) => {
    ...
    async fn pop_batch_jobs(
        self: &Arc<Self>,
        batch_size: usize,
    ) -> Result<Vec<BorrowedJob<H::JobData>>, MessageQueueError> {
        ...
        let script = redis::Script::new(
            ...
            for i = 1, #active_jobs, 2 do
                ...
                if current_lease_token then
                    ...
                    local lease_key = 'queue:' .. queue_id .. ':job:' .. job_id .. ':lease:' .. current_lease_token
            ...
            for _, job_id in ipairs(popped_job_ids) do
                ...
                if job_data then
                    ...
                    local lease_key = 'queue:' .. queue_id .. ':job:' .. job_id .. ':lease:' .. lease_token
        ...
        let job_results: Vec<(String, String, String, String, String, String)> =
            lua_json_array(&job_results_json)?;
        let cancelled_jobs: Vec<String> = lua_json_array(&cancelled_jobs_json)?;
        let timed_out_jobs: Vec<String> = lua_json_array(&timed_out_jobs_json)?;
                ...
                if #job_ids_to_delete > 0 then
                    for _, j_id in ipairs(job_ids_to_delete) do
                        ...
                        if not is_active and not is_pending and not is_delayed then
                            local job_meta_hash = 'queue:' .. queue_id .. ':job:' .. j_id .. ':meta'
                            local errors_list_name = 'queue:' .. queue_id .. ':job:' .. j_id .. ':errors'
    ...
    pub async fn remove_from_dedupe_set(&self, job_id: &str) -> Result<(), MessageQueueError> {
        self.redis
            .clone()
    ...
    pub async fn empty_dedupe_set(&self) -> Result<(), MessageQueueError> {
        self.redis
            .clone()
