...
pub struct WorkerHandle<Q> {
    pub join_handle: tokio::task::JoinHandle<Result<(), MessageQueueError>>,
...
impl<Q: QueueIdentifier> WorkerHandle<Q> {
    ...
    pub async fn shutdown(self) -> Result<(), MessageQueueError> {
        tracing::info!(
...
pub struct ShutdownHandle {
    join_handles: Vec<tokio::task::JoinHandle<Result<(), MessageQueueError>>>,
    ...
    pub async fn shutdown(self) -> Result<(), MessageQueueError> {
        ...
        for (i, result) in results.into_iter().enumerate() {
            match result {
                ...
                Err(e) => {
                    let runtime_error = MessageQueueError::Runtime {
