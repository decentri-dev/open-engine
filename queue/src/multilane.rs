...
impl<H: DurableExecution> MultilaneQueue<H> {
    ...
    pub fn lanes_zset_name(&self) -> String {
        format!("queue_multilane:{}:lanes", self.queue_id)
    }

    pub fn lane_pending_list_name(&self, lane_id: &str) -> String {
        format!("queue_multilane:{}:lane:{}:pending", self.queue_id, lane_id)
    }

    pub fn lane_delayed_zset_name(&self, lane_id: &str) -> String {
        format!("queue_multilane:{}:lane:{}:delayed", self.queue_id, lane_id)
    }

    pub fn lane_active_hash_name(&self, lane_id: &str) -> String {
        format!("queue_multilane:{}:lane:{}:active", self.queue_id, lane_id)
    }

    pub fn success_list_name(&self) -> String {
        format!("queue_multilane:{}:success", self.queue_id)
    }

    pub fn failed_list_name(&self) -> String {
        format!("queue_multilane:{}:failed", self.queue_id)
    }

    pub fn job_data_hash_name(&self) -> String {
        format!("queue_multilane:{}:jobs:data", self.queue_id)
    }

    pub fn job_meta_hash_name(&self, job_id: &str) -> String {
        format!("queue_multilane:{}:job:{}:meta", self.queue_id, job_id)
    }

    pub fn job_errors_list_name(&self, job_id: &str) -> String {
        format!("queue_multilane:{}:job:{}:errors", self.queue_id, job_id)
    }

    pub fn job_result_hash_name(&self) -> String {
        format!("queue_multilane:{}:jobs:result", self.queue_id)
    }

    pub fn dedupe_set_name(&self) -> String {
        format!("queue_multilane:{}:dedup", self.queue_id)
    }

    pub fn pending_cancellation_set_name(&self) -> String {
        format!("queue_multilane:{}:pending_cancellations", self.queue_id)
    }

    pub fn lease_key_name(&self, job_id: &str, lease_token: &str) -> String {
        format!(
            "queue_multilane:{}:job:{}:lease:{}",
    ...
    pub async fn get_job(
        &self,
        job_id: &str,
    ) -> Result<Option<Job<H::JobData>>, MessageQueueError> {
    ...
    pub async fn lanes_count(&self) -> Result<usize, MessageQueueError> {
        let mut conn = self.redis.clone();
    ...
    pub async fn cancel_job(&self, job_id: &str) -> Result<CancelResult, MessageQueueError> {
        let script = redis::Script::new(
            r#"
            local queue_id = ARGV[1]
            local job_id = ARGV[2]
            ...
            local lane_pending_list = 'queue_multilane:' .. queue_id .. ':lane:' .. lane_id .. ':pending'
            local lane_delayed_zset = 'queue_multilane:' .. queue_id .. ':lane:' .. lane_id .. ':delayed'
            local lane_active_hash = 'queue_multilane:' .. queue_id .. ':lane:' .. lane_id .. ':active'
        ...
        match result.as_str() {
            ...
            _ => Err(MessageQueueError::Runtime {
    ...
    pub fn work(self: &Arc<Self>) -> WorkerHandle<MultilaneQueue<H>> {
        ...
        let join_handle = tokio::spawn(async move {
                            ...
                            Ok(jobs) => {
                                ...
                                for (lane_id, job) in jobs {
                                    ...
                                    tokio::spawn(async move {
                                        ...
                                        drop(permit);
                                    }.instrument(tracing::info_span!("queue_multilane_worker", job_id, lane_id)));
                                }
                            }
                            Err(e) => {
            ...
            let _permits: Vec<_> = (0..outer_queue_clone.options.local_concurrency)
                ...
                .map_err(|e| MessageQueueError::Runtime {
    ...
    pub async fn pop_batch_jobs(
        self: &Arc<Self>,
        batch_size: usize,
    ) -> Result<Vec<(String, BorrowedJob<H::JobData>)>, MessageQueueError> {
        let script = redis::Script::new(
            r#"
            local queue_id = ARGV[1]
            ...
            local function cleanup_lane_leases(lane_id)
                ...
                for i = 1, #active_jobs, 2 do
                    ...
                    if current_lease_token then
                        local lease_key = 'queue_multilane:' .. queue_id .. ':job:' .. job_id .. ':lease:' .. current_lease_token
            ...
            local function pop_job_from_lane(lane_id)
                ...
                local lease_key = 'queue_multilane:' .. queue_id .. ':job:' .. job_id .. ':lease:' .. lease_token
            ...
            if total_lanes == 0 then
                return {
                    cjson.encode(result_jobs),
                    cjson.encode(cancelled_jobs),
                    cjson.encode(timed_out_jobs)
                }
            end
        ...
        let (job_results_json, cancelled_jobs_json, timed_out_jobs_json): (String, String, String) =
            script
                .key(self.lanes_zset_name())
                .key(self.job_data_hash_name())
                .key(self.pending_cancellation_set_name())
        ...
        let job_results: Vec<(String, String, String, String, String, String, String)> =
            lua_json_array(&job_results_json)?;
        let cancelled_jobs: Vec<(String, String)> = lua_json_array(&cancelled_jobs_json)?;
        let timed_out_jobs: Vec<(String, String)> = lua_json_array(&timed_out_jobs_json)?;
            ...
            match serde_json::from_str::<H::JobData>(&job_data_t_json) {
                ...
                Err(e) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let queue_error: MessageQueueError = e.into();

                        if let Err(e) = queue_clone
                            .complete_job_queue_error(&job, &lease_token, &queue_error.into())
                            .await
                        {
                            tracing::error!(
                                job_id = job.id,
    ...
    async fn process_cancelled_job(&self, job_id: &str) -> Result<(), MessageQueueError> {
            ...
            r#"
                ...
                if #job_ids_to_delete > 0 then
                    for _, j_id in ipairs(job_ids_to_delete) do
                        ...
                        local job_meta_hash = 'queue_multilane:' .. queue_id .. ':job:' .. j_id .. ':meta'
                        local lane_id = redis.call('HGET', job_meta_hash, 'lane_id')
                        ...
                        if should_delete then
                            local errors_list_name = 'queue_multilane:' .. queue_id .. ':job:' .. j_id .. ':errors'
    ...
    async fn post_fail_completion(&self) -> Result<(), MessageQueueError> {
        let trim_script = redis::Script::new(
            r#"
                local queue_id = KEYS[1]
                local list_name = KEYS[2]
    ...
    async fn complete_job_queue_error(
        ...
        let queue_error = MessageQueueError::Runtime {
            message: "Job processing failed with user error".to_string(),
        };
        let queue_error_hook_data = QueueInternalErrorHookData {
            error: &queue_error,
        };
        self.handler
...
impl<H: DurableExecution> MultilanePushableJob<H> {
    ...
    pub async fn push(self) -> Result<Job<H::JobData>, MessageQueueError> {
