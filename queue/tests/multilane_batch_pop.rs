...
use queue::job::{BorrowedJob, JobResult};
use queue::{DurableExecution, MultilaneQueue, UserCancellable};
...
impl From<MessageQueueError> for TestError {
...
impl MultilaneTestHarness {
    async fn new() -> Self {
        ...
        let _ = queue.count(queue::job::JobStatus::Active, None).await;
    ...
    async fn cleanup(&self) {
        ...
        let keys_pattern = format!("queue_multilane:{}:*", self.queue_id);
    ...
    async fn count_total_jobs(&self, status: queue::job::JobStatus) -> usize {
        self.queue
            .count(status, None)
            .await
    ...
    async fn count_lane_jobs(&self, lane_id: &str, status: queue::job::JobStatus) -> usize {
        self.queue
...
impl Drop for MultilaneTestHarness {
    fn drop(&mut self) {
        ...
        tokio::spawn(async move {
            let mut conn = redis;
            let keys_pattern = format!("queue_multilane:{queue_id}:*");
            let keys: Vec<String> = redis::cmd("KEYS")
                .arg(&keys_pattern)
                .query_async(&mut conn)
                .await
...
async fn multilane_test_batch_pop_single_lane_with_100k_empty_lanes() {
    ...
    let pending_count = harness
        .count_total_jobs(queue::job::JobStatus::Pending)
        .await;
    assert_eq!(pending_count, 100, "Should have 100 pending jobs");

    let active_lane_count = harness
        .count_lane_jobs(&active_lane, queue::job::JobStatus::Pending)
        .await;
    assert_eq!(
