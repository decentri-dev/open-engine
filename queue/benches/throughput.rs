...
use queue::job::JobError;
use queue::{BorrowedJob, IdempotencyMode, UserCancellable};
use rand::Rng;
...
impl From<MessageQueueError> for BenchmarkErrorData {
...
fn find_max_throughput(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("queue_max_throughput");
...
fn nack_percentage_impact(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("queue_nack_impact");
...
fn basic_throughput(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("queue_basic_1k_jobs");
